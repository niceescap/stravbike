...
from api.routes import (
    auth, athlete, activities, sessions, competitions,
    comments, llm, calendar, web, chat,
)
...
# ── Routes web (pages HTML via Jinja2) ──────────────────────
# Pas d'auth service key : la clé est injectée côté serveur dans les templates.
app.include_router(web.router, tags=["web"])


# ── Routes API (consommées par frontend ET outil OpenWebUI) ──
# Auth X-API-Key sur tous les routers API.
...
app.include_router(chat.router, prefix="/api/chat", tags=["chat"], dependencies=_API_DEPS)
